"""Greenhouse Job Board 公开接口。只落 JSON 快照，不做职位解析。

GET /v1/boards/{board_token}/jobs?content=true 免鉴权。
content 为 HTML 实体转义后的正文，解析时由 postings mapper unescape。
"""
...
BOARDS_FILE = Path(__file__).with_name("greenhouse_boards.txt")
            ...
            for token, board_name in self._boards:
                ...
                for job in jobs:
                    ...
                    if since is not None:
                        job_day = _iso_date(job.get("updated_at") or job.get("first_published"))
