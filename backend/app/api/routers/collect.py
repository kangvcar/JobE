...
def _collectors(max_items: int, delay: float, liepin_enabled: bool) -> dict:
    limiter = RateLimiter(delay)
    out: dict = {
        "mohrss": MohrssCollector(limiter=limiter, max_items=max_items, delay_seconds=delay),
        "moka": MokaCollector(limiter=limiter, max_items=max_items, delay_seconds=delay),
    }
    if liepin_enabled:
...
def trigger_collect(
    ...
    return run_collect(
        collectors=_collectors(cap, settings.collect_delay_seconds, settings.liepin_enabled),
